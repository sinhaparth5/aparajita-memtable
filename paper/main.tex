...
\title{Aparajita: Branchless SIMD Search and Append-Only\\
       Nodes for LSM-Tree MemTables}
...
skip list with a list of cache-line-sized nodes, each holding sixteen 32-bit
order-preserving key surrogates searched by a branchless SIMD kernel. Three
...
without which six of seven realistic key distributions collapse to a single
surrogate value across 200{,}000 keys. Nodes are append-only, and the sorted order
...
39\% faster than the skip list at 1, 4, 16 and 64 threads, with non-overlapping
samples across ten runs per configuration, backed by 55\% fewer retired
instructions and 40\% fewer L1 misses per lookup. The skip list is 48\% to 52\% slower
at the representation level, and Aparajita is 18.8\% faster in single-threaded
\texttt{db\_bench},
...
line. If sixteen keys fit in one 64-byte line and can be compared in one or two
vector instructions, then a search over those sixteen keys costs one cache fill,
...
\item A cache-line node layout with a branchless ordered SIMD search kernel whose
      ...
      probe on AVX-512 and 5.10 on AVX2 against 27.65 for a branchy scalar
      baseline, with zero branch mispredictions per probe.
\item A prefix-relative order-preserving surrogate, motivated by a measurement
      showing that six of seven realistic key distributions give an absolute
      leading-bytes surrogate no discriminating power at all.
...
\item An eight-byte descent hint in the node header that replaces a virtual
      ...
      loaded.
\item An evaluation against RocksDB's skip list and VectorRep that separates the
      representation's cost from the harness's, including two measurement errors
      we made and corrected, and an exit criterion we could not meet.
\end{itemize}
...
what that costs VectorRep in practice.
...
Aparajita differs from this line of work in what it is embedded in rather than in
the kernel itself. These structures are read-optimised indexes built once or
...
A node holds sixteen 32-bit surrogates in exactly one 64-byte cache line, aligned
so it never straddles a line boundary. AVX2 covers the line in two 256-bit
compares and AVX-512 in one, so a probe costs one line fill and a fixed number of
instructions independent of where the answer lies.

Everything else the node needs, the order word, the shared prefix length, the
level-0 successor pointer and the sixteen full key pointers, begins at offset 64.
...
unpredictable; measured, it cost 0.5 branch mispredictions per probe and roughly
three quarters of the AVX2 kernel's cycles, taking it from 25.4 to 6.2 cycles
per probe. Folding absence into the compare mask
...
is what established it. Across seven synthetic but realistic key distributions at
200{,}000 keys each, six collapsed to a single distinct surrogate value: a table
name prefix, a tenant identifier or a big-endian timestamp puts identical bytes at
the front of every key in a database. A lane that holds the same value for every
key discriminates nothing, and the SIMD kernel degenerates to a linear scan
confirmed key by key.
...
saturates in the direction it sorts.
...
not instrument the vector load.
...
thing differing between columns is the representation. Five repetitions per cell,
and the read and write passes fill independently, giving ten samples per fill
cell.
...
\begin{table}[t]
\caption{Read throughput, kops/s, WAL off. VectorRep is capped at 20
         operations per run and reported separately in
         Section~\ref{sec:vectorrep}.}
\label{tab:reads}
\centering
\begin{tabular}{lrrrr}
\toprule
...
\midrule
\multicolumn{5}{l}{\texttt{readrandom}} \\
1  &   731.9 &  528.2 & --- & +38.6\% \\
4  &  2703.1 & 1969.1 & --- & +37.3\% \\
16 &  8773.9 & 7181.8 & --- & +22.2\% \\
64 & 10913.5 & 8677.9 & --- & +25.8\% \\
\midrule
\multicolumn{5}{l}{\texttt{readwhilewriting}} \\
1  &   705.6 &  519.7 & --- & +35.8\% \\
4  &  2693.8 & 1950.2 & --- & +38.1\% \\
16 &  8691.9 & 6738.8 & --- & +29.0\% \\
64 & 10531.9 & 8578.5 & --- & +22.8\% \\
\bottomrule
\end{tabular}
\end{table}

Aparajita is ahead at every thread count on both benchmarks, and the samples do
not overlap: across ten runs per cell, Aparajita's slowest run beats the skip
list's fastest at every point but one.
...
\midrule
\texttt{readwhile-} & Aparajita &  3622.2 &  9.53 & 1.265 &  72.41 \\
\texttt{writing}    & skip list &  7597.6 & 11.93 & 0.808 & 110.70 \\
\bottomrule
\end{tabular}
\end{table}
...
within 3\%, which is the useful thing they say. Its \texttt{readrandom} shows the
same read advantage narrowing under oversubscription, from 62.5\% at one thread to
0.4\% at 64.
...
40\% fewer L1 misses per lookup. The kernel work is necessary and far from
